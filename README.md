...
！！！！！！注意，是带有“服务器”关键字的exe文件，另一个exe文件是第一个版本的，直接无视！！！！也不要下载后和服务器版本的同时运行

1. **下载EXE文件**：`dist/失物招领系统-服务器.exe和对应数据库文件`（这个数据库文件含之前存储的数据，如果不复制数据库文件，运行exe文件他会在当前目录下生成一个新的数据库文件）
2. **双击运行**：EXE文件
3. **访问系统**：根据服务器exe终端界面里的实际地址，复制到浏览器打开 
